 ...
 * dedicated thread.
 *
 * Pure POSIX C (no GLib, no Audacious headers): the reader thread is a plain
 * pthread, which keeps this file hermetically testable (tests/feed_test.c) and
 * free of the host's C++ plugin API.
 */
 ...
 * is the adapter's).
 *
 * A session is touched from two threads, so the feed object is
 * reference-counted:
 *
 * - Audacious runs a general plugin's init()/cleanup() on the program's main
 *   thread (libaudcore/plugin.h:78-80: "Single-thread plugins: visualization,
 *   general, and interface. Functions provided by these plugins will only be
 *   called from the main thread"), and every hook this adapter listens to is
 *   delivered on that same thread: playlist.cc:207-209 fires "playback
 *   begin"/"playback stop" inside Playlist::process_pending_update(), which is
 *   queued on the main loop (playlist.cc:217/242), and playback.cc:484/268/326
 *   event_queue() the pause/unpause/seek hooks. All host->app pushes, and the
 *   whole session lifetime (init() spawns, cleanup() stops), therefore live on
 *   that one thread.
 * - the feed's own reader thread runs the callbacks below — including
 *   lx_feed_send_spectrum() from the analyser callback, so the writers really
 *   are called from two threads.
 *
