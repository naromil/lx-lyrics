...
QString embeddedLyrics(const Fooyin::Track& track)
{
  ...
  for (const QString& prefix : suffixedPrefixes) {
    ...
    const auto it =
      std::find_if(extraTags.cbegin(), extraTags.cend(), [&prefix](const auto& extraTag) {
        return extraTag.first.startsWith(prefix) && !extraTag.second.isEmpty();
      });
    if (it != extraTags.cend()) {
...
QString readSidecar(const Fooyin::Track& track)
{
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    qWarning() << "[LX Lyrics] cannot open local lyrics sidecar" << candidate << "for track"
               << filepath;
    return QString();
  }
