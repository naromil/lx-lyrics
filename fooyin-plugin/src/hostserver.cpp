...
HostServer::HostServer(QObject* parent)
    : QObject(parent)
    , m_server(new QWebSocketServer(QStringLiteral("lx-lyrics"), QWebSocketServer::NonSecureMode, this))
{
...
{
    QJsonObject object{{QStringLiteral("v"), 1}, {QStringLiteral("action"), QStringLiteral("set_info")}};
...
void HostServer::sendSetLyric(const QString& lrc, const QString& tlrc, const QString& rlrc,
                              const QString& lxlrc)
{
    sendJson(QStringLiteral("set_lyric"),
             {{QStringLiteral("lrc"), lrc},
