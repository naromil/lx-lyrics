...
PlayerBridge::PlayerBridge(Fooyin::PlayerController* playerController, HostServer* host, QObject* parent)
    : QObject(parent)
...
void PlayerBridge::onPlayStateChanged(Fooyin::Player::PlayState state, Fooyin::Player::PlayState previous)
{
    Q_UNUSED(previous)
    if (!m_pushing) {
        return;
...
{
    return m_playerController != nullptr ? static_cast<qint64>(m_playerController->currentPosition()) : 0;
}

void PlayerBridge::currentLyrics(const Fooyin::Track& track, QString& lrc, QString& tlrc, QString& rlrc,
                                 QString& lxlrc) const
{
...
QVariantMap PlayerBridge::setInfoFields(const Fooyin::Track& track, bool isPlay, qint64 playedTime,
                                        const QString& lrc, const QString& tlrc, const QString& rlrc,
