...
void LxLyricsPlugin::initialise(const Fooyin::GuiPluginContext& context)
{
    ...
    m_actionManager->registerAction(m_toggleAction, Fooyin::Id(QStringLiteral("LxLyrics.DesktopLyrics")));
    ...
    qInfo() << "[LX Lyrics] GuiPlugin initialised; actionManager stored; 'Desktop Lyrics' toggle added to View menu";
}
...
void LxLyricsPlugin::startDesktopLyrics()
{
    if (m_hostServer == nullptr) {
        ...
        connect(m_hostServer.get(), &HostServer::clientDisconnected, this, &LxLyricsPlugin::onClientDisconnected);
        ...
        m_playerBridge->setLyricProvider([](const Fooyin::Track& track, QString& lrc, QString& tlrc,
        ...
        m_spectrumSource = std::make_unique<SpectrumSource>(m_engineController, m_hostServer.get(), this);
