...
const QStringList kUnplayPresets = {
  QStringLiteral("rgba(255, 255, 255, 1)"), QStringLiteral("rgba(255, 236, 144, 1)"),
  QStringLiteral("rgba(144, 255, 206, 1)"), QStringLiteral("rgba(32, 255, 132, 1)"),
  QStringLiteral("rgba(255, 226, 32, 1)"),  QStringLiteral("rgba(57, 203, 255, 1)"),
  QStringLiteral("rgba(217, 57, 255, 1)"),  QStringLiteral("rgba(255, 57, 71, 1)"),
};

const QStringList kPlayedPresets = {
  QStringLiteral("rgba(255, 236, 144, 1)"), QStringLiteral("rgba(144, 255, 206, 1)"),
  QStringLiteral("rgba(32, 255, 132, 1)"),  QStringLiteral("rgba(255, 226, 32, 1)"),
  QStringLiteral("rgba(57, 203, 255, 1)"),  QStringLiteral("rgba(7, 197, 86, 1)"),
  QStringLiteral("rgba(25, 181, 254, 1)"),  QStringLiteral("rgba(217, 57, 255, 1)"),
...
SettingsDialog::SettingsDialog(DesktopLyricConfig& config, TranslationManager& i18n,
                               QWidget* parent)
  : QDialog(parent)
  , m_config(config)
  , m_i18n(i18n)
...
QRadioButton* SettingsDialog::addRadio(QWidget* parent, const QString& label, const QString& key,
                                       const QString& value)
{
...
QWidget* SettingsDialog::buildGeneralGroup(QWidget* parent)
{
  ...
  for (const CheckboxSpec& spec : generalCheckboxSpecs()) {
    layout->addWidget(addCheckbox(group, m_i18n.tr(QString::fromUtf8(spec.labelKey)),
                                  QString::fromUtf8(spec.configKey)));
  }
  // False positive: the layout is parented to `group` (QLayout(QWidget*) ctor), so it is
...
{
  auto* group =
    new QGroupBox(m_i18n.tr(QStringLiteral("setting__desktop_lyric_font_weight")), parent);
  auto* layout = new QHBoxLayout(group);
  layout->addWidget(
    addCheckbox(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_font_weight_font")),
                QStringLiteral("desktopLyric.style.isFontWeightFont")));
  layout->addWidget(
    addCheckbox(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_font_weight_line")),
                QStringLiteral("desktopLyric.style.isFontWeightLine")));
  layout->addWidget(
    addCheckbox(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_font_weight_extended")),
                QStringLiteral("desktopLyric.style.isFontWeightExtended")));
  layout->addStretch();
  return group;
}
...
{
  auto* group =
    new QGroupBox(m_i18n.tr(QStringLiteral("setting__desktop_lyric_direction")), parent);
  auto* layout = new QHBoxLayout(group);
  layout->addWidget(
    addRadio(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_direction_horizontal")),
             QStringLiteral("desktopLyric.direction"), QStringLiteral("horizontal")));
  layout->addWidget(addRadio(group,
                             m_i18n.tr(QStringLiteral("setting__desktop_lyric_direction_vertical")),
                             QStringLiteral("desktopLyric.direction"), QStringLiteral("vertical")));
  layout->addStretch();
  return group;
}
...
{
  auto* group =
    new QGroupBox(m_i18n.tr(QStringLiteral("setting__desktop_lyric_scroll_align")), parent);
  auto* layout = new QHBoxLayout(group);
  layout->addWidget(addRadio(group,
                             m_i18n.tr(QStringLiteral("setting__desktop_lyric_scroll_align_top")),
                             QStringLiteral("desktopLyric.scrollAlign"), QStringLiteral("top")));
  layout->addWidget(
    addRadio(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_scroll_align_center")),
...
QWidget* SettingsDialog::buildAlignGroup(QWidget* parent)
{
  ...
  auto* layout = new QHBoxLayout(group);
  layout->addWidget(addRadio(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_align_left")),
                             QStringLiteral("desktopLyric.style.align"), QStringLiteral("left")));
  layout->addWidget(addRadio(group,
                             m_i18n.tr(QStringLiteral("setting__desktop_lyric_align_center")),
                             QStringLiteral("desktopLyric.style.align"), QStringLiteral("center")));
  layout->addWidget(addRadio(group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_align_right")),
                             QStringLiteral("desktopLyric.style.align"), QStringLiteral("right")));
  layout->addStretch();
  return group;
}
...
QWidget* SettingsDialog::buildStyleGroup(QWidget* parent)
{
  ...
  auto* resetSpacingButton =
    new QPushButton(m_i18n.tr(QStringLiteral("setting__desktop_lyric_reset")), group);
  ...
  connect(resetSpacingButton, &QPushButton::clicked, this, [this] {
    m_lineGapSpin->setValue(15);
  });
  ...
  m_fontCombo->addItem(m_i18n.tr(QStringLiteral("setting__desktop_lyric_font_default")),
                       QString()); // Empty family = default font.
  ...
  connect(m_fontCombo, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int index) {
    m_config.set(QStringLiteral("desktopLyric.style.font"),
                 m_fontCombo->itemData(index).toString());
  });
...
void SettingsDialog::updateLineGapLabel()
{
  ...
  m_lineGapLabel->setText(
    m_i18n.tr(QStringLiteral("setting__desktop_lyric_line_gap"))
      .replace(
        QStringLiteral("{num}"),
...
QWidget* SettingsDialog::buildColorGroup(QWidget* parent)
{
  ...
  m_unplayButton =
    new ColorPickerButton(QStringLiteral("desktopLyric.style.lyricUnplayColor"), m_config, m_i18n,
                          kUnplayPresets, kDefaultUnplayColor, group);
  layout->addWidget(makeColorColumn(
    group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_unplay_color")), m_unplayButton));

  m_playedButton =
    new ColorPickerButton(QStringLiteral("desktopLyric.style.lyricPlayedColor"), m_config, m_i18n,
                          kPlayedPresets, kDefaultPlayedColor, group);
  layout->addWidget(makeColorColumn(
    group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_played_color")), m_playedButton));

  m_shadowButton =
    new ColorPickerButton(QStringLiteral("desktopLyric.style.lyricShadowColor"), m_config, m_i18n,
                          kShadowPresets, kDefaultShadowColor, group);
  layout->addWidget(makeColorColumn(
    group, m_i18n.tr(QStringLiteral("setting__desktop_lyric_shadow_color")), m_shadowButton));
...
QWidget* SettingsDialog::buildResetGroup(QWidget* parent)
{
  ...
  m_resetWindowButton =
    new QPushButton(m_i18n.tr(QStringLiteral("setting__desktop_lyric_reset_window")), group);
...
void SettingsDialog::populateFromConfig()
{
  ...
  }
  {
    ...
    const int index =
      m_fontCombo->findData(m_config.get(QStringLiteral("desktopLyric.style.font")).toString());
