...
protected:
    void paintEvent(QPaintEvent*) override
    {
        ...
        const QColor border = (underMouse() || isDown())
...
public:
    ColorPickerDialog(const QColor& current,
        ...
        , m_i18n(i18n)
    {
        ...
        connect(moreButton, &QPushButton::clicked, this, [this, current] {
            const QColor picked = QColorDialog::getColor(
                current, this, QStringLiteral("Select color"), QColorDialog::ShowAlphaChannel);
            if (picked.isValid()) {
                m_selected = picked;
                accept();
            }
...
ColorPickerButton::ColorPickerButton(const QString& configKey,
...
void ColorPickerButton::paintEvent(QPaintEvent*)
{
    ...
    const QColor border = (isDown() || hasFocus())
        ? palette().color(QPalette::Highlight)
