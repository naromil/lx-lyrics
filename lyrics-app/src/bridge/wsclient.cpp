...
bool parseTrackSnapshot(const QJsonObject& obj, TrackSnapshot* out, QString* error)
{
    return parseStringField(obj, QStringLiteral("id"), &out->id, error)
        ...
        && parseStringField(obj, QStringLiteral("lrc"), &out->lrc, error)
        && parseStringField(obj, QStringLiteral("tlrc"), &out->tlrc, error)
        && parseStringField(obj, QStringLiteral("rlrc"), &out->rlrc, error)
        && parseStringField(obj, QStringLiteral("lxlrc"), &out->lxlrc, error)
        && parseBoolField(obj, QStringLiteral("isPlay"), &out->isPlay, error)
        && parseInt64Field(obj, QStringLiteral("line"), &out->line, error)
        && parseInt64Field(obj, QStringLiteral("played_time"), &out->playedTimeMs, error);
}

bool parseLyricSnapshot(const QJsonObject& obj, LyricSnapshot* out, QString* error)
{
    return parseStringField(obj, QStringLiteral("lrc"), &out->lrc, error)
        && parseStringField(obj, QStringLiteral("tlrc"), &out->tlrc, error)
        && parseStringField(obj, QStringLiteral("rlrc"), &out->rlrc, error)
        && parseStringField(obj, QStringLiteral("lxlrc"), &out->lxlrc, error);
}
...
{
    return parseBoolField(obj, QStringLiteral("isPlay"), &out->isPlay, error)
        && parseInt64Field(obj, QStringLiteral("line"), &out->line, error)
        && parseInt64Field(obj, QStringLiteral("played_time"), &out->playedTimeMs, error);
}
    ...
    , m_reconnectIntervalMs(reconnectIntervalMs)
{
    ...
    connect(&m_socket, &QWebSocket::stateChanged,
            this, &WsClient::onSocketStateChanged);
    ...
    connect(&m_socket, &QWebSocket::textMessageReceived,
            this, &WsClient::dispatchIncomingText);
    connect(&m_socket, &QWebSocket::binaryMessageReceived,
            this, &WsClient::dispatchIncomingBinary);
}
...
void WsClient::dispatchIncomingBinary(const QByteArray& frame)
{
    ...
    if (frame.size() != kAnalyserFrameBytes) {
        qWarning() << "WsClient: dropping analyser frame of" << frame.size()
                   << "bytes (expected" << kAnalyserFrameBytes << ")";
        return;
    }
