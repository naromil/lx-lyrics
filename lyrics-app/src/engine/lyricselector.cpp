...
const QRegularExpression kAwlrcVerifyRxp(QStringLiteral("(?:^|\\s*)\\[\\d+:\\d+(?:\\.\\d+)]<\\d+,\\d+>.+$"),
...
AwlrcParts parseAwlrcContent(const QString& content)
{
    ...
    parts.found = !parts.lyric.isEmpty() || !parts.tlyric.isEmpty()
        || !parts.rlyric.isEmpty() || !parts.lxlyric.isEmpty();
    return parts;
}
...
void LyricSelector::setLyrics(const QString& lrc, const QString& tlyric,
