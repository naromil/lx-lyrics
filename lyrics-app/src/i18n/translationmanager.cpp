...
constexpr auto kSupportedLanguages = { kLanguageChineseSimplified,
...
void TranslationManager::applyLanguage()
{
    ...
    if (!loadPack(kLanguageEnglish, &m_enPack)) {
        qWarning() << "TranslationManager: en-us pack unavailable; keys will fall through to the key itself";
        m_enPack.clear();
    }
...
{
    const QString configured = m_config.get(QLatin1String(kKeyLanguageId)).toString().trimmed();
