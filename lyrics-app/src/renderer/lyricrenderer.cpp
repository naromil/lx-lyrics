...
{
  static const QRegularExpression kKaraokeTagRxp(QStringLiteral("<\\d+,\\d+>"));
...
constexpr std::array<StrokeOffset, 32> kStroke3Offsets = {{
  {0.04, true, 0.04, true},   {0.04, true, -0.03, true}, {-0.04, true, -0.03, true},
  {-0.04, true, 0.04, true},  {0.04, true, 0.01, true},  {0.04, true, -0.01, true},
  {-0.04, true, -0.01, true}, {-0.04, true, 0.01, true}, {0.04, true, 0.00, true},
  {0.04, true, 0.00, true},   {-0.04, true, 0.00, true}, {-0.04, true, 0.00, true},
  {0.01, true, 0.04, true},   {0.01, true, -0.03, true}, {-0.01, true, -0.03, true},
  {-0.01, true, 0.04, true},  {0.01, true, 0.01, true},  {0.01, true, -0.01, true},
  {-0.01, true, -0.01, true}, {-0.01, true, 0.01, true}, {0.01, true, 0.00, true},
  {0.01, true, 0.00, true},   {-0.01, true, 0.00, true}, {-0.01, true, 0.00, true},
  {0.00, true, 0.04, true},   {0.00, true, -0.03, true}, {0.00, true, -0.03, true},
  {0.00, true, 0.04, true},   {0.00, true, 0.01, true},  {0.00, true, -0.01, true},
...
QColor interpolateColor(const QColor& unplay, const QColor& played, double p)
{
  ...
  if (p >= 1.0)
    return played;
  return QColor(qRound(unplay.red() + (played.red() - unplay.red()) * p),
...
QStringList wrapForHeight(const QString& text, const QFontMetrics& fm, int availableHeight,
                          int letterSpacing, qreal scale = 1.0)
{
...
void LyricRenderer::setActiveLine(int index)
{
  ...
  const int clamped =
    (index >= 0 && index < m_lines.size() && !m_lines[index].isStatic) ? index : -1;
...
void LyricRenderer::startLineTransition(QVector<LineTransition>& transitions,
                                        const QVector<double>& progress, int line, double to)
{
...
qreal LyricRenderer::mainZoomFactor(int lineIndex, double zoomOverride) const
{
  ...
  const qreal progress = (zoomOverride >= 0.0) ? zoomOverride
...
QString LyricRenderer::elideForWidth(const QString& text, const QFontMetrics& fm,
                                     int availableWidth, qreal scale) const
{
...
QString LyricRenderer::elideForHeight(const QString& text, const QFontMetrics& fm,
                                      int availableHeight, int letterSpacing, qreal scale) const
{
...
qreal LyricRenderer::columnHeight(const QString& text, const QFontMetrics& fm, int letterSpacing,
                                  qreal scale) const
{
  if (text.isEmpty())
    return 0;
...
LyricRenderer::VerticalGroupMetrics LyricRenderer::measureVerticalGroup(const RenderLine& line,
                                                                        int lineIndex,
...
void LyricRenderer::renderLineToCache(int lineIndex, const QRectF& rect,
                                      const VerticalGroupMetrics* verticalMetrics)
{
  ...
  const QRectF local(kCacheHaloPadPx, kCacheHaloPadPx, rect.width(), rect.height());
  if (verticalMetrics)
    drawVerticalGroup(cachePainter, *verticalMetrics, lineIndex, local);
  else
    drawHorizontalGroup(cachePainter, m_lines.at(lineIndex), lineIndex, local);
  cache.dirty = false;
}

void LyricRenderer::drawHorizontalGroup(QPainter& p, const RenderLine& line, int lineIndex,
                                        const QRectF& rect)
{
  ...
  if (m_ellipsis) {
    ...
    const QString mainText =
      elideForWidth(stripWordTags(line.text), mainFm, int(rect.width()), mainScale);
    ...
    for (const QString& rawExtended : line.extended) {
      ...
      const QString extText =
        elideForWidth(stripWordTags(rawExtended), extendedFm, int(rect.width()), extScale);
      drawTextWithStroke(p, extText,
                         QRectF(rect.x(), y, rect.width(), extendedFm.height() * extScale),
  ...
  qreal y = rect.y();
  const QStringList mainRows =
    wrapForWidth(stripWordTags(line.text), mainFm, int(rect.width()), mainScale);
  ...
  for (const QString& rawExtended : line.extended) {
    ...
    const QStringList extRows =
      wrapForWidth(stripWordTags(rawExtended), extendedFm, int(rect.width()), extScale);
...
void LyricRenderer::drawVerticalGroup(QPainter& p, const VerticalGroupMetrics& m, int lineIndex,
                                      const QRectF& rect)
{
  const QFontMetrics mainFm(m.mainFont);
  const QFontMetrics extendedFm(m.extendedFont);
  ...
  auto drawColumns = [&](const QStringList& cols, const QFont& font, const QFontMetrics& fm,
    ...
    for (int i = 0; i < cols.size(); ++i) {
      ...
      drawVerticalText(p, cols.at(i), colRect, font, fill, m_shadowColor, StrokeStyle::Stroke4,
                       letterSpacing, scale);
      right = colRect.left();
      if (i < cols.size() - 1)
        right -= letterSpacing;
...
void LyricRenderer::drawVerticalText(QPainter& p, const QString& text, const QRectF& columnRect,
                                     const QFont& font, const QColor& fill, const QColor& stroke,
                                     StrokeStyle style, int letterSpacing, qreal scale)
{
  if (text.isEmpty())
    return;
...
void LyricRenderer::drawTextWithStroke(QPainter& p, const QString& text, const QRectF& rect,
                                       const QFont& font, const QColor& fill, const QColor& stroke,
                                       StrokeStyle style, qreal scale)
{
  ...
  const StrokeOffset* offsets =
    (style == StrokeStyle::Stroke3) ? kStroke3Offsets.data() : kStroke4Offsets.data();
  const std::size_t passCount =
    (style == StrokeStyle::Stroke3) ? kStroke3Offsets.size() : kStroke4Offsets.size();
...
LyricRenderer::HorizontalLayout LyricRenderer::measureHorizontal(int zoomOverrideLine,
                                                                 double zoomOverride) const
{
...
qreal LyricRenderer::measureLineGroupHeight(int lineIndex, double zoomOverride) const
{
  ...
  if (m_ellipsis) {
    ...
    if (!line.extended.isEmpty())
      groupHeight +=
        line.extended.size() * (horizontalExtendedGap(m_lineGap) + extendedFm.height() * extScale);
  } else {
    groupHeight += wrapForWidth(stripWordTags(line.text), mainFm, wrapWidth, mainScale).size() *
                   mainFm.height() * mainScale;
    for (const QString& rawExtended : line.extended)
      groupHeight +=
        horizontalExtendedGap(m_lineGap) +
        wrapForWidth(stripWordTags(rawExtended), extendedFm, wrapWidth, extScale).size() *
...
LyricRenderer::measureAllVerticalGroups(int zoomOverrideLine, double zoomOverride) const
{
...
qreal LyricRenderer::maxScrollOffset() const
{
  ...
  const qreal blockExtent =
    m_vertical ? verticalBlockWidth(measureAllVerticalGroups()) : measureHorizontal().blockHeight;
...
qreal LyricRenderer::autoScrollTarget() const
{
  ...
  if (m_vertical) {
    ...
    const qreal target =
      m_scrollAlignTop ? (width() - groupExtent - 2) : (width() - groupExtent) / 2.0;
  ...
  if (m_zoomActiveLrc && !m_delayScroll && m_prevActiveLine >= 0 &&
    ...
    const qreal baseHeight =
      measureHorizontal(m_prevActiveLine, 0.0).groupHeights.at(m_prevActiveLine);
