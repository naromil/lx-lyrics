...
public:
  ...
  void setFontWeightFont(
    bool
      on); // write-only: no longer affects rendering (line-by-line mode); retained for API compatibility
  ...
  void setShadowFontModeColor(
    const QColor&
      c); // write-only: no longer affects rendering (line-by-line mode); retained for API compatibility
  ...
  void setDelayScroll(
    bool on); // false=immediate 300ms animation (default), true=600ms delay + 600ms animation
  void
  setUserScrolling(bool on); // suspend auto-scroll while true; re-arm the resume timer when false
  ...
  {
    return (line >= 0 && line < m_lineColorProgress.size()) ? m_lineColorProgress.at(line) : 0.0;
  }
  double zoomProgressForLine(int line) const
  {
    return (line >= 0 && line < m_lineZoomProgress.size()) ? m_lineZoomProgress.at(line) : 0.0;
  }
...
private:
  ...
  void drawVerticalGroup(QPainter& p, const VerticalGroupMetrics& metrics, int lineIndex,
                         const QRectF& rect);
  void drawVerticalText(QPainter& p, const QString& text, const QRectF& columnRect,
                        const QFont& font, const QColor& fill, const QColor& stroke,
                        StrokeStyle style, int letterSpacing, qreal scale = 1.0);
  ...
  QString elideForWidth(const QString& text, const QFontMetrics& fm, int availableWidth,
                        qreal scale = 1.0) const;
  QString elideForHeight(const QString& text, const QFontMetrics& fm, int availableHeight,
                         int letterSpacing, qreal scale = 1.0) const;
  qreal columnWidth(const QString& text, const QFontMetrics& fm, qreal scale = 1.0) const;
  qreal columnHeight(const QString& text, const QFontMetrics& fm, int letterSpacing,
                     qreal scale = 1.0) const;
  ...
  int m_activeLine =
    -1; // current active line; setLines resets to -1 so the first active-line event re-positions the view
  ...
  bool m_delayScroll =
    false; // false=immediate 300ms animation (default), true=600ms delay + 600ms animation
  ...
  void startLineTransition(QVector<LineTransition>& transitions, const QVector<double>& progress,
                           int line, double to);
