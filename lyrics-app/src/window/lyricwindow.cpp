  ...
  , m_i18n(i18n)
{
  ...
  connect(m_fadeAnim, &QVariantAnimation::valueChanged, this, [this](const QVariant& value) {
  ...
  connect(m_paneAnim, &QVariantAnimation::valueChanged, this, [this](const QVariant& value) {
  ...
  connect(m_nativeOpSaveTimer, &QTimer::timeout, this, [this] {
    saveBounds();
  });
  ...
  connect(&m_config, &DesktopLyricConfig::settingChanged, this, &LyricWindow::applySetting);
  ...
  connect(settingsShortcut, &QShortcut::activated, this, &LyricWindow::openSettingsDialog);
...
void LyricWindow::openSettingsDialog()
{
  ...
  if (!alreadyVisible && !(m_settingsDialog->windowFlags() & Qt::WindowStaysOnTopHint)) {
...
{
  m_hoverHideActive =
    m_config.get(QStringLiteral("desktopLyric.isHoverHide")).toBool() && m_config.isLock();
...
void LyricWindow::paintEvent(QPaintEvent*)
{
  ...
  // static kPaneFill.
  const QColor paneFill =
    QColor(kPaneFill.red(), kPaneFill.green(), kPaneFill.blue(), qRound(m_paneAlpha * 255));
...
void LyricWindow::applySetting(const QString& key, const QVariant& value)
{
  if (key == QStringLiteral("desktopLyric.isLock")) {
    ...
    resizeToConfigSize();
  } else if (key == QStringLiteral("desktopLyric.x") || key == QStringLiteral("desktopLyric.y")) {
    applySavedPosition();
...
bool LyricWindow::intersectsAnyScreen(const QPoint& topLeft, const QSize& size) const
{
  ...
  return std::any_of(screens.cbegin(), screens.cend(), [&windowRect](const QScreen* screen) {
    return screen->availableGeometry().intersects(windowRect);
  });
}

QPoint LyricWindow::clampedTopLeft(const QPoint& topLeft, const QSize& windowSize,
                                   const QRect& available) const
{
  ...
  return QPoint(qBound(available.x(), topLeft.x(), maxX), qBound(available.y(), topLeft.y(), maxY));
}
