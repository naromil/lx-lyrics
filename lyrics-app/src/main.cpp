...
TrackSnapshot makeDemoTrack()
{
    ...
    track.lrc = QStringLiteral(
        "[00:00.00]Demo line one\n"
        "[00:02.00]<0,1500>This <1500,1500>line has word tags\n"
        "[00:05.00]Plain third line");
    track.tlrc = QStringLiteral(
        "[00:00.00]First line translation\n"
        "[00:02.00]<0,1200>醒目 <1200,800>金色\n"
        "[00:05.00]Third line translation");
    track.rlrc = QStringLiteral(
        "[00:00.00]First rōmaji\n"
...
int main(int argc, char* argv[])
{
    ...
    if (!cli.wsUrl.isEmpty()) {
        appContext.wsClient = std::make_unique<WsClient>(WsClient::kDefaultReconnectIntervalMs, &appContext);
        ...
        QObject::connect(ws, &WsClient::pauseReceived, &appContext,
                         [&appContext, lyricController] {
                             lyricController->pause();
                             appContext.pauseHide->setPlayState(false);
                         });
        QObject::connect(ws, &WsClient::stopReceived, &appContext,
                         [&appContext, lyricController] {
        ...
        appContext.spectrumBridge = std::make_unique<SpectrumBridge>(
            window->spectrumWidget(), ws, appContext.config, &appContext);
