 ...
 * The "app" is a POSIX shell stub, so no DeaDBeeF, player or display server is
...
static void test_scripted_lines(const char* script, const char* log_path)
{
  ...
  lx_feed_t* feed = lx_feed_spawn(script, "deadbeef", true, &callbacks, NULL);
  ...
  snprintf(expected, sizeof(expected),
           "{\"v\":2,\"action\":\"hello\",\"host\":\"deadbeef\",\"spectrum\":true}\n"
...
static void test_periodic_status(const char* script, const char* log_path)
{
  ...
  lx_feed_t* feed = lx_feed_spawn(script, "deadbeef", false, &callbacks, NULL);
  ...
  while (*cursor) {
    ...
    *end = '\0';
    index++;
    const char* expected = index == 1 ? "{\"v\":2,\"action\":\"hello\",\"host\":\"deadbeef\","
                                        "\"spectrum\":false}"
...
static void test_no_fd_leak(const char* script, const char* log_path)
{
  ...
  lx_feed_t* warmup = lx_feed_spawn(script, "deadbeef", false, &callbacks, NULL);
  if (!warmup) {
...
{
  child_exits = 0;
  check(lx_feed_spawn(NULL, "deadbeef", false, NULL, NULL) == NULL,
  ...
  const lx_feed_callbacks_t callbacks = {.child_exited = on_child_exited};
  lx_feed_t* feed =
    lx_feed_spawn("/nonexistent/lx-lyrics-app", "deadbeef", false, &callbacks, NULL);
                                           ...
                                           const char* log_path)
{
  ...
  lx_feed_t* warm = lx_feed_spawn(drain_stub, "deadbeef", false, &callbacks, NULL);
  if (!warm) {
  ...
  lx_feed_t* feed = lx_feed_spawn(eof_stub, "deadbeef", false, &callbacks, NULL);
  if (!feed) {
...
static void test_concurrent_stop(const char* drain_stub)
{
  ...
  for (int cycle = 0; cycle < 10 && threads_ok; cycle++) {
    lx_feed_t* feed = lx_feed_spawn(drain_stub, "deadbeef", false, &callbacks, NULL);
    if (!feed) {
...
static void test_noise_is_logged_loudly(const char* noise_stub, const char* capture_path)
{
  ...
  const lx_feed_callbacks_t callbacks = {.child_exited = on_child_exited};
  lx_feed_t* feed = lx_feed_spawn(noise_stub, "deadbeef", false, &callbacks, NULL);
...
int main(void)
{
  ...
  const char* const stub_paths[] = {
    ...
    bad_v_script, bad_action_script, overlong_script,
  };
  const char* const stub_bodies[] = {
    k_stub_script,       k_drain_stub_script,      k_eof_stub_script,      k_noise_stub_script,
    k_bad_v_stub_script, k_bad_action_stub_script, k_overlong_stub_script,
  };
