...
/* How long the child gets to leave on its own after the session ended. */
...
/*
 * write() to a pipe whose reader is gone raises SIGPIPE, which would kill the
 * player. Block it around the call and swallow the signal we generated instead
 * of changing the process-wide disposition from inside a plugin.
 */
 ...
 * the player's main thread). Returns false when the app is gone or too slow. */
 ...
 * non-NULL the body is copied, escapes included verbatim (the two v2 actions
 * carry none, and the copy is only compared against them). */
...
static bool feed_handle_line(struct lx_feed* feed, const char* line, size_t length)
{
  ...
  if (strcmp(action, "get_analyser_data_array") == 0) {
    if (feed->callbacks.analyser_data_requested) {
      feed->callbacks.analyser_data_requested(feed, feed->userdata);
    }
    return true;
  }
                         ...
                         const lx_feed_callbacks_t* callbacks, void* userdata)
{
  ...
  /* hello first: protocol.md §3 makes it the very first host->app line. */
  lx_buf_t hello = {0};
  buf_printf(&hello, "{\"v\":%d,\"action\":\"hello\",\"host\":\"", LX_FEED_PROTOCOL_VERSION);
  buf_put_escaped(&hello, host_name ? host_name : "");
  buf_printf(&hello, "\",\"spectrum\":%s}", spectrum_available ? "true" : "false");
...
void lx_feed_send_spectrum(lx_feed_t* feed, const uint8_t* bytes, size_t size)
{
  ...
  if (size != LX_FEED_SPECTRUM_BYTES) {
    lx_log("refusing a spectrum frame of %zu bytes (protocol.md §5: exactly %d)", size,
           LX_FEED_SPECTRUM_BYTES);
    return;
  }
  lx_buf_t line = {0};
  buf_printf(&line, "{\"v\":%d,\"action\":\"spectrum\",\"data\":\"", LX_FEED_PROTOCOL_VERSION);
  size_t encoded_size = ((LX_FEED_SPECTRUM_BYTES + 2) / 3) * 4 + 1;
  buf_reserve(&line, encoded_size);
  if (!line.oom) {
    size_t written = lx_base64_encode(bytes, size, line.data + line.len, line.cap - line.len);
    line.len += written;
  }
  buf_put(&line, "\"}");
  feed_emit(feed, &line, "spectrum");
}
